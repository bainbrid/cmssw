...
double LowPtGsfElectronIDProducer::eval(const std::string& name,
  ...
  if (iter != names_.end()) {
    ...
    if      ( version_ == "V0" ) { inputs = lowptgsfeleid::features_V0(ele, rho, unbiased); }
    else if ( version_ == "V1" ) { inputs = lowptgsfeleid::features(ele, rho, unbiased); }
