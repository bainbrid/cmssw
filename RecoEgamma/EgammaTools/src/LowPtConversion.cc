...
bool LowPtConversion::wpLoose() {
  if (wpOpen() && ntracks == 2 && valid == true && quality_high_purity == true && chi2prob > 0.0005) {
    return true;
  } else {
    return false;
  }
...
bool LowPtConversion::wpTight() {
  if (wpLoose() && sum_nhits_before_vtx <= 1 && l_xy > 0. && mass_from_conv > 0. &&  // sanity check
      mass_from_conv < 0.05) {
    return true;
  } else {
    return false;
...
void LowPtConversion::addUserVars(pat::Electron& ele) {
...
void LowPtConversion::addExtraUserVars(pat::Electron& ele) {
...
////////////////////////////////////////////////////////////////////////////////
//
bool LowPtConversion::match(const edm::Handle<reco::BeamSpot>& beamSpot,
                            const edm::Handle<edm::View<reco::Conversion> >& conversions,
                            const pat::Electron& ele) {
  LowPtConversion info;
  return LowPtConversion::match(beamSpot, conversions, ele, info);
}
...
bool LowPtConversion::match(const edm::Handle<reco::BeamSpot>& beamSpot,
  ...
  if (!(beamSpot.isValid())) {
    ...
    os << "Problem accessing BeamSpot collection" << std::endl;
    throw cms::Exception("InvalidHandle", os.str());
  }
  if (!(conversions.isValid())) {
    std::ostringstream os;
    os << "Problem accessing Conversions collection" << std::endl;
    throw cms::Exception("InvalidHandle", os.str());
  }
  ...
  for (const auto& conv : *conversions) {
    ...
    for (const auto& trk : conv.tracks()) {
      if (trk.isNonnull() && trk.isAvailable() && (info.min_trk_pt < 0. || trk->pt() < info.min_trk_pt)) {
        info.min_trk_pt = trk->pt();
      }
    }
    info.ilead = -1;
    info.itrail = -1;
    ...
    if (conv.tracks().size() >= 2 && info.ilead > -1 && info.itrail > -1) {
      ...
      info.mass_before_fit = mee(lead_before_vtx_fit->px(),
                                 lead_before_vtx_fit->py(),
                                 lead_before_vtx_fit->pz(),
                                 trail_before_vtx_fit->px(),
                                 trail_before_vtx_fit->py(),
                                 trail_before_vtx_fit->pz());
    }

    // Invariant mass after the fit to common vertex
    if (conv.conversionVertex().refittedTracks().size() >= 2 && info.ilead > -1 && info.itrail > -1) {
      ...
      info.mass_after_fit = mee(lead_after_vtx_fit.px(),
    ...
    if (info.ilead > -1 && info.itrail > -1) {
      ...
      info.max_nhits_before_vtx =
          conv.nHitsBeforeVtx().size() > 1
              ? (conv.nHitsBeforeVtx().at(0) > conv.nHitsBeforeVtx().at(1) ? conv.nHitsBeforeVtx().at(0)
                                                                           : conv.nHitsBeforeVtx().at(1))
              : 0;
      info.sum_nhits_before_vtx =
          conv.nHitsBeforeVtx().size() > 1 ? conv.nHitsBeforeVtx().at(0) + conv.nHitsBeforeVtx().at(1) : 0;
    }
